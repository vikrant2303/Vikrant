package Test.com.or.cache.implementation;
...
public class CacheManager {
			...
			Thread threadCleanerUpper = new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						while (true) {
							System.out
									.println("ThreadCleanerUpper Scanning For Expired Objects...");
							/*
							...
							while (keys.hasNext()) {
								...
								if (value.isExpired()) {
									...
									System.out
											.println("ThreadCleanerUpper Running.Found an Expired Object in the Cache.");
								}
							}
							/*
							 * ***********************************************************************
	...
	public static void putCache(Cacheable object) {
		// Remember if the HashMap previously contains a mapping for the key,the
		// old value
